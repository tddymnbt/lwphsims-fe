...
export default function ItemDetailPage() {
  ...
  useEffect(() => {
    const fetchMovements = async () => {
      ...
      try {
        const endpoint = `${API_BASE_URL}/products/id/${product.stock_external_id}/transactions`;
        const requestBody = {
  ...
  const handleSaveStock = async () => {
    ...
    try {
      ...
      const endpoint = `${API_BASE_URL}/products/update-stock/id/${product.stock_external_id}`;
      const payload = {
      ...
      if (response.data.status?.success) {
        ...
        setTimeout(async () => {
          ...
          setIsLoading(true);
          try {
            const refreshed = await axios.get(`${API_BASE_URL}/products/id/${id}`);
